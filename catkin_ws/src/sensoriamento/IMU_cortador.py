...
import numpy as np
import smbus # Instalar na raspberry 

try:
    from adxl345 import ADXL345 # Instalar na raspberry 
    from hmc5883l import HMC5883L # Instalar na raspberry 
    from bmp085 import BMP085 # Instalar na raspberry 
    from l3g4200d import L3G4200D # Instalar na raspberry 
    from i2cutils import i2c_raspberry_pi_bus_number # Instalar na raspberry 
except ImportError:
...
class IMU(object):
    def __init__(self, bus=None):
        ...
        #Leitura padrão do acelerometro
        ...
        self._last_gyro_time = 0 #variável para interpretar as informações do giróscópio
        self.read_gyro_delta() # Avalia a diferença entre os valores do giroscópio
    ...
    def update(self):
        ...
        if delta_t < 0.020:
            #intervalo de 20ms entre novas atualizações
            return
        ...
        #Calculo do giróscopio separado
        rot_mag = sqrt(sum(v_gyro**2)) #magnitude do vetor rotação
        ...
        if abs(sqrt(sum(v_acc**2)) - 1) < 0.3:
            #Identificar a direçao da gravidade ("para baixo")
            v_down = v_acc * -1.0
            v_east = np.cross(v_down, v_mag) #Leste é o cros product do vetor pra baixo e pro norte
            v_north = np.cross(v_east, v_down)# Aponta na mesma direção do magnetômetro
            # formação de uma base orthogonal de referência. (Vetor/magnitude)
            v_down /= sqrt((v_down**2).sum()) 
            v_east /= sqrt((v_east**2).sum())
            v_north /= sqrt((v_north**2).sum())
            #Filtragem
            #Ruido da orientação de magnetometro e acelerometro, 2%
            #Considerando erro de variação do gyroscópio, 98%
    ...
    def current_orientation_quaternion_hybrid(self):
        """Direção baseado nas direções norte(mag), leste, baixo(acelerometro) """
        self.update()
    ...
    def compute_orientation(self):
        self.update()
        w, x, y, z = self._current_hybrid_orientation_q
        self.yaw, self.pitch, self.roll= quaternion_to_euler_angles(w, x, y, z)
        return
